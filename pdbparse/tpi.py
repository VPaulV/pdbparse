...
def resolve_typerefs(leaf, types, min):
    ...
    #TODO: do we need handler here for simple types?
    if leaf.leaf_type not in type_refs:
        #print("skipped key 0x{:02x}".format(int(leaf.leaf_type)))
        return leaf

    for attr in type_refs[leaf.leaf_type]:
        ref = getattr(leaf, attr, None)
        if ref == None:
            continue
...
def rename_2_7(lf):
    if type(lf.leaf_type) != EnumIntegerString:
        return
